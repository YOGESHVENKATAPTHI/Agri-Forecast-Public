...
export async function chatWithAI(userMessage: string, chatHistory: AIMessage[], userId?: string): Promise<string | null> {
  return await chatWithEnhancedAI(userMessage, chatHistory, userId || 'anonymous');
}
...
export async function chatWithEnhancedAI(
  ...
  try {
    console.log(`🤖 Enhanced AI chat request for user ${userId}${landId ? ` (land ${landId})` : ''} with comprehensive data`);
...
Use YYYY-MM-DD for dates or null. Keep reasoning under 50 chars, plan under 100 chars. Do not include any text outside the JSON array.`
    ...
    for (const crop of recommendations.slice(0, 5)) { // Limit to 5 recommendations
      try {
        ...
        const savedCrop = await storage.saveLandCropRecommendation({ 
          landId: landId,
          userId: landData.userId,
...
export async function generateLandDroughtPrediction(landId: number) {
  try {
    ...
    const savedDroughtPrediction = await storage.saveLandDroughtPrediction({ 
      landId: landId,
      userId: landData.userId,
    ...
    const landData = await storage.getLandById(landId);
    const fallbackPrediction = await storage.saveLandDroughtPrediction({ 
      landId: landId,
      userId: landData?.userId || 'unknown',
      riskLevel: 'moderate',
      probability: 35,
